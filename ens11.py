...
for i in range(30):
    ...
    Xtrain, Xtest, ytrain, ytest = train_test_split(x_train, y_train, test_size=0.1,random_state=i)
...
output = pd.DataFrame({'id': id_test, 'price_doc11':average})
output.head()

output_train=pd.DataFrame({'price_doc11':averageTrain})
output.to_csv('predictionTrain11.csv',index=False)


# In[97]:

output.to_csv('predictionTest11.csv', index=False)
