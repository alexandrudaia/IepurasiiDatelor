...
for i in range(30):
    ...
    Xtrain, Xtest, ytrain, ytest = train_test_split(x_train, y_train, test_size=0.2,random_state=i)
    ...
    y_pred=(y_predict1+y_predict2)/2
    prediction_list.append(y_predict)
    pred_train1=model1.predict(xgb.DMatrix(x_train))
    pred_train2=model2.predict(xgb.DMatrix(x_train))
    pred_train=(pred_train1+pred_train2)/2
...
output = pd.DataFrame({'id': id_test, 'price_doc14':average})
output.head()

output_train=pd.DataFrame({'price_doc14':averageTrain})
output.to_csv('predictionTrain14.csv',index=False)


# In[97]:

output.to_csv('predictionTest14.csv', index=False)
